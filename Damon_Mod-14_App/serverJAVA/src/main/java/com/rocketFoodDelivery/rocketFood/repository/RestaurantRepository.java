...
public interface RestaurantRepository extends JpaRepository<Restaurant, Integer> {
    ...
    /**
     * Finds a restaurant by its ID along with the calculated average rating rounded
     * up to the ceiling.
     *
     * @param restaurantId The ID of the restaurant to retrieve.
     * @return A list of Object arrays representing the selected columns from the
     *         query result.
     *         Each Object array corresponds to the restaurant's information.
     *         An empty list is returned if no restaurant is found with the
     *         specified ID.
     */
    @Query(nativeQuery = true, value = "SELECT r.id, r.name, r.price_range, r.active, " +
     ...
     *
     * Executes a native SQL query that retrieves restaurants with their
     * information, including a calculated
     * average rating rounded up to the ceiling.
     *
     * @param rating     The minimum rounded-up average rating of the restaurants.
     *                   (Optional)
    ...
    @Modifying
    @Transactional
    @Query(nativeQuery = true, value = "INSERT INTO restaurants (user_id, address_id, name, price_range, phone, email, active) "
            +
    ...
    @Modifying
    @Transactional
    @Query(nativeQuery = true, value = "UPDATE restaurants " +
